...
class Actividad {
    ...
    public function cargarActividad($mostrarDetalles) {
        ...
        $consulta = "SELECT id_texto,intentos,iniciada,fecha_inicio,fecha_fin
                FROM actividades_por_usuario 
                WHERE id_usuario=" . DB::limpiarSQL($this->id_usuario) . "
    ...
    public function registrarIntentoActividad() {
        ...
        $condicion = "id_usuario=" . DB::limpiarSQL($this->id_usuario) .
                " AND id_actividad='" . DB::limpiarSQL($this->id_actividad) . "'";
    ...
    private function cargarDatosInterfaz($mostrarDetalles) {
        ...
        if ($mostrarDetalles) {
            $consulta = "SELECT nombre,mostrar_texto_cifrado,id_metodo,tipo_respuesta,forma_respuesta,mostrar_clave 
                FROM actividades 
                WHERE id_actividad =" . DB::limpiarSQL($this->id_actividad);
        } else {
